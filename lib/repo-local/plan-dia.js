...
function isDueOnOrBeforeToday(v, todayYMD) {
    if (!isValidDateLike(v)) return false;
    const ymd = ymdInTZ(new Date(v), 'America/Bogota');
    return ymd <= todayYMD;
}
 ...
 *   PERO solo si ya fueron estudiadas alguna vez (nivelActual > 0 o fechaUltimoEstudio).
 * - Nuevas = frases con nivelActual=0 y sin fechaUltimoEstudio (ignorando revisarEnFecha).
...
export async function getPlanDelDiaLocal({ limit, maxOrden, excludeIds = [] }) {
    ...
    // Solo considerar como "due" lo que ya fue estudiado alguna vez
    const due = todasFrasesValidas
        .filter(f => !excludeSet.has(String(f._id || f.id)))
        .filter(f => ((f.nivelActual ?? 0) > 0) || !!f.fechaUltimoEstudio)
        .filter(f => isDueOnOrBeforeToday(f.revisarEnFecha, todayYMD));

    // ---- Pool de NUEVAS ----
    // Ignorar revisarEnFecha por completo para nuevas
